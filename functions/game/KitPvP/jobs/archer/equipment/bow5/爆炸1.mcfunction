#只有爆炸
scoreboard players tag @s[tag=book:爆炸,score_花粉_min=50000] add ench:爆炸
...
scoreboard players remove @s[tag=ench:爆炸] 花粉 50000
give @s[tag=ench:爆炸] minecraft:bow 1 0 {boom:1b,bow:1,archer:1b,bow:5,display:{Name:"§6Lv5 §c轩辕弓",Lore:["§7爆炸","§6〓〓〓〓〓〓〓〓","§a神匠打造给轩辕帝的弓","§c可升级，自身附魔、命名不继承"]},ench:[{id:48s,lvl:8s},{id:50s,lvl:5s},{id:49s,lvl:5s},{id:70s,lvl:1s}]}
playsound minecraft:block.anvil.use ambient @s[tag=ench:爆炸]
tellraw @s[tag=ench:爆炸] [{"text":"§6【Archer】  §a新装备附魔成功!"}]
#打造失败
execute @s[tag=book:爆炸] ~ ~ ~ tellraw @s[tag=!ench:爆炸] [{"text":"§6【Archer】  §c花粉不足5w打造失败!"}]
