...
scoreboard players tag @s[score_temp_min=61] add $prefixFail
scoreboard players tag @s[score_temp=0] add $prefixFail
tellraw @s[tag=$prefixHad] [{"text":"您已经购买了此称号！","color":"red"}]
tellraw @s[tag=$prefixFail] [{"text":"必须设定正确的temp值以选择欲购买的称号","color":"red"}]

execute @s[tag=!$prefixHad] ~ ~ ~ function ch:buy/had if @s[tag=!$prefixFail]

scoreboard players tag @s remove $prefixHad
scoreboard players tag @s remove $prefixBuy
scoreboard players tag @s remove $prefixFail
